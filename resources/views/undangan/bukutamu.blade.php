        ...
        https://blanchedalmond-elk-754408.hostingersite.com/wedding/{{ $order ? $order->domain : '' }}?to=
                                                        ...
                                                        <div class="card-footer d-flex justify-content-end"
                                                            ...
                                                            <button type="button" class="btn btn-md me-1"
                                                                style="border-radius:10px; background-color:#9ebf68; color:white;"
                                                                id="btn-share">
                                                                Share
                                                            </button>
            ...
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Nama Group (Opsional)</label>
                            <input type="text" name="group" id="group-edit" class="form-control"
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group mb-3">
                                <span class="input-group-text" id="basic-addon1">+62</span>
                                <input type="number" name="whatsapp" id="whatsapp-edit" class="form-control"
                        ...
                        <div class="form-group">
                            <label>Jumlah Tamu (Opsional)</label>
                            <div class="input-group mb-3">
                                <input type="number" class="form-control" name="jumlah" placeholder="Jumlah Tamu"
                                    id="jumlah-edit">
                                <span class="input-group-text" id="basic-addon1">Orang</span>
                            </div>
                            <div class="invalid-feedback"></div>
...
@section('script')
    <script>
        ...
        const baseUrl = 'https://blanchedalmond-elk-754408.hostingersite.com/wedding/{{ $order ? $order->domain : '' }}?to=';
        
        // 2. Ambil nama tamu dari input dan pastikan aman untuk URL
        const guestName = encodeURIComponent($(this).val());
        
        // 3. Gabungkan menjadi URL lengkap yang baru
        const newURL = baseUrl + guestName;
    ...
    // 1. Validasi: Pastikan nama tamu tidak kosong
    if (!nama) {
        Swal.fire('Peringatan', 'Nama tamu tidak boleh kosong untuk membuat link.', 'warning');
        return;
    }

    // 2. Ambil template pesan dari atribut data
    const text = $('#btn-copy').data('text');

    // 3. Buat link undangan yang benar dan dinamis
    //    Ini adalah bagian utama yang diperbaiki
    const linkUndangan = 'https://blanchedalmond-elk-754408.hostingersite.com/wedding/{{ $order ? $order->domain : '' }}?to=' + encodeURIComponent(nama);

    // 4. Ganti semua variabel di dalam template
    const pesanLinkUndangan = text
        .replace(/{nama_tamu}/g, nama)
        .replace(/{nama_group}/g, group || '') // Fallback jika group null
        .replace(/{kode_tamu}/g, kodetamu || '') // Fallback jika kode tamu null
        .replace(/{link_undangan}/g, linkUndangan);

    // 5. Encode seluruh pesan agar aman untuk URL WhatsApp
    //    Ini lebih aman daripada hanya mengganti \n
    const convertedValue = encodeURIComponent(pesanLinkUndangan);

    // 6. Buka link WhatsApp di tab baru
    if (whatsapp) {
        // Jika nomor WhatsApp ada, kirim langsung ke nomor tersebut
        window.open(`https://api.whatsapp.com/send/?phone=62${whatsapp}&text=${convertedValue}`, '_blank');
    } else {
        // Jika tidak ada nomor, buka WhatsApp untuk dibagikan secara manual
        window.open(`https://api.whatsapp.com/send/?text=${convertedValue}`, '_blank');
    }
}
